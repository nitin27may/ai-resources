...
!!! note
    For a 1,000-document corpus you might expect 5,000–20,000 LLM calls during indexing, depending on document length and entity density. At mid-tier hosted model prices this has typically landed in the tens to low
    hundreds of dollars per full index build, and it scales with corpus size and
    entity density rather than with query volume. Price it against the model you
    will actually use, on a sample, before indexing everything — and note that
    every re-index pays it again, so a corpus that changes daily changes the
